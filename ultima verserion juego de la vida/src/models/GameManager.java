...
public class GameManager {
    ...
    public void generateFootsxDD(int Time) {
        Thread thread1 = new Thread(new Runnable() {
            @Override
            public void run() {
                while (true) {
    ...
    public void generateInitialIndividuals(int xQuantity, int yQuantity) {
        generateIndividuals(xQuantity, Gender.X);
        generateIndividuals(yQuantity, Gender.Y);
    }

    private void generateIndividuals(int quantity, Gender g) {
    	Random r = new Random();
        int energy = r.nextInt(10-1)+1;
        for (int i = 0; i < quantity; i++) {
            ...
            individuals.put(individualsCount, new Individual(x, y, individualsCount, g, energy,
                    stagesOfLife.get(2)));
            individualsCount++;
        }
    }
        ...
        for(Integer individualKey : individuals.keySet()){
            ...
            for(Integer nextIndividualKey : individuals.keySet()){
                if (individuals.get(individualKey).getId() != individuals.get(nextIndividualKey).getId()) {
                    if (verifyCoordinates(individuals.get(individualKey).getX(), individuals.get(individualKey).getY(), individuals.get(nextIndividualKey).getX(), individuals.get(nextIndividualKey).getY())) {
    ...
    private void verifyCollisionConditions(Individual individual1, Individual individual2) {
        if (individual1.getGender().equals(individual2.getGender())) {
            if (individual1.getGender().equals(Gender.X) || individual2.getGender().equals(Gender.X)  || individual1.getGender().equals(Gender.Y) || individual2.getGender().equals(Gender.Y) ) {
    ...
    private void verifyXYCollisions(Individual individual1, Individual individual2) {
        if (individual1.getStageOfLife().getName().equals(stagesOfLife.get(2).getName()) && individual2.getStageOfLife().getName().equals(stagesOfLife.get(2).getName())) {
    ...
    private void generateNewIndividual() {
        ...
        int y = (int) (Math.random() * Utilities.height);
        Random r = new Random();
        Gender gender = (r.nextBoolean() ? Gender.X : Gender.Y);
        individuals.put(individualsCount, new Individual(x, y, individualsCount, gender, 20, stagesOfLife.get(0)));
        individualsCount++;
    }
    ...
    public void getstageLife() {
    	for (int i = 0; i < this.stagesOfLife.size(); i++) {
			System.out.println(stagesOfLife.get(i));
		}
    }
}